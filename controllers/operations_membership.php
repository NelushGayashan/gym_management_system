<?php 
   ...
   require_once('C:/xampp/htdocs/test/views/common/dbconfig.php'); 
    ...
    class  operations_membership extends dbconfig
    {
		...
		$fee = $db->check($_POST['fee']);
                
        

                if($this->insert_record($id,$name,$description,$fee))
                {
        ...
        function insert_record($id,$name,$description,$fee)
        {
            global $db;
            $query = "insert into membership (id,name,description,fee) values('$id','$name','$description','$fee')";
        ...
        {
            global $db;
            $query = "select * from membership ORDER BY id DESC";
            $result = mysqli_query($db->connection,$query);
            return $result;
        }
        ...
        public function update()
        {
            ...
            if(isset($_POST['btn_update']))
            {
                ...
                if($this->update_record($id,$name,$description,$fee))
                {
        ...
        // Update Function 2
        public function update_record($id,$name,$description,$fee)
        {
            global $db;
            $sql = "update membership set name='$name', description='$description',fee = '$fee' where ID='$id'";
